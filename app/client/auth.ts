...
export class JWT {
   ...
   */
  getToken(data: any = 'hey', key: any = null, expiration: number = 3600, mode: string = 'bytes'): any {
    
    let keyObj;
    if (typeof key === 'string' || key === null) {
    } else {
      if (cryptoType !== key.getCryptoType(key.cryptoType)) {
  ...
  verifyToken(token: string | any): any {
    ...
    if (!this.key.verify(message, signature, data.iss)) {
